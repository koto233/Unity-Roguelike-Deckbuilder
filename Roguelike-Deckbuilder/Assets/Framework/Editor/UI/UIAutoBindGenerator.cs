...
namespace LitFramework.UI.Core.Utility
{
    public static class UIAutoBindGenerator
    {
        ...
        /// </summary>
        /// <param name="ui"></param>
        public static void CollectAndGenerate(UIBase ui)
        {
            if (ui == null) return;
            // 收集 UIBind，但跳过子 UIBase 的子树
            var binds = new List<UIBind>();
            CollectBindsExcludingSubUI(ui.transform, ui, binds);
        ...
        {
            // 1. 先检查当前节点上有没有 UIBind（无论是根还是子模块，都先收了再说）
            var bind = current.GetComponent<UIBind>();
            if (bind != null && current != rootUI.transform)
                result.Add(bind);

            // 2. 检查当前节点是不是一个“子模块”（UIBase 且不是自身）
            var uiBase = current.GetComponent<UIBase>();
            if (uiBase != null && uiBase != rootUI)
            {
                // 它是子模块：收集完它根节点上的 UIBind 后，立即停止深入，
                // 不去碰它内部的 Image、Button 等子物体
                return;
            }

            // 3. 如果不是子模块（或是根自己），则继续递归遍历所有子物体
        ...
        static void GenerateCode(UIBase ui, UIBind[] binds)
        {
            var type = ui.GetType();

...
public partial class MapView : UIWindow
{
    private AssetRef<GameObject> _nodePrefabRef;
    private AssetRef<GameObject> _linePrefabRef;
    private List<Transform> _rows = new();
    private List<MapNodeItem> _nodes = new();
    private List<UIMapLine> _lines = new();
    private Dictionary<string, MapNodeItem> _nodeDict = new Dictionary<string, MapNodeItem>();
    ...
    private MapNodeItem CreateNode(MapNodeData data, Transform parent)
    {
        // 1. 从池里拿对象（你已经在 Clear 里回池了，这里也要用池拿）
        var nodeGO = _poolService.GetGameObject<MapNodeItem>();
    ...
    private void DrawLine(MapNodeItem from, MapNodeItem to)
    {

...
public partial class HandZone : MonoBehaviour
{
    [SerializeField] private FanLayout _fanLayout;
    [SerializeField] private Transform _handContainer;
    [SerializeField] private Transform _cardDetailTrans;
    private readonly Dictionary<int, HandCard> _cardItems = new();
    private ObjectPoolService _poolService;
    private BattleView _battleWindow;
        ...
        ChangeType type)
    {
        ...
        switch (type)
        {
            case ChangeType.Remove:
                List<HandCard> removedItems = new List<HandCard>();
    ...
    /// 从池中获取一个卡牌实例并初始化
    /// </summary>
    private HandCard CreateCard(Card card)
    {
        var go = _poolService.GetGameObject<HandCard>();
        go.SetActive(true);
    ...
    private async UniTask PlayDiscardAnimations(List<HandCard> cards)
    {
    ...
    private async UniTask PlayAddAnimations(IReadOnlyList<Card> changedCards)
    {
        ...
        var flyTasks = new List<UniTask>();
        var flyObjs = new List<GameObject>();
        var newItems = new List<HandCard>();
    ...
    private void ClearHandContainer()
    {
        for (int i = _handContainer.childCount - 1; i >= 0; i--)
        {
            ...
            var item = child.GetComponent<HandCard>();
            if (item != null)
                ReturnCardToPool(item);
            else
    ...
    private void ReturnCardToPool(HandCard item)
    {
        if (item == null) return;
        item.gameObject.SetActive(false);
        item.transform.localScale = Vector3.one;
        item.transform.localRotation = Quaternion.identity;
        _poolService.ReturnGameObject<HandCard>(item.gameObject);
    }
